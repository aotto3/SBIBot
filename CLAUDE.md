...
- **90-day picker horizon:** bookeo-asst ignores the `to` param and always returns ~1 week from `from` (same quirk noted above), so a single `getSchedule` call can't reach further than ~7 days. `bookeo.getScheduleForDays(from, days)` in `lib/bookeo.js` stitches a longer horizon from parallel 7-day-chunk `getSchedule` calls (each still hits the normal 5-min per-call cache) via pure helpers `_buildChunkRanges` (chunk boundaries) + `_mergeScheduleChunks` (dedupe by date+time+show, clip to `[from, to]` — chunk edges can overshoot). `/coverage-request`'s picker and `/member-schedule` both use it (90-day horizon, no `week_of` option — `/member-schedule` shows a member's full upcoming schedule in one reply, clamped to Discord's 2000-char content limit). `/schedule` is unchanged (still single-call, ~7 days).
