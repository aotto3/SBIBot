...
- Tests: `npm test` (runs all `test/*.test.js` — 16 files, 339 tests). Uses `--test-force-exit`: `checkin.js` schedules real (non-`unref`'d) `setTimeout` alerts, so without it the runner hangs after tests finish waiting on pending timers.
