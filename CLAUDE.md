...
- bookeo-asst: `/api/schedule` scopes the range via `start`/`end` query params — NOT `from`/`to` (which it silently ignores, returning its default ~7-day window). `getSchedule(from, to)` keeps its inclusive-`to` contract but on the wire sends `start=from` and `end=to+1 day` because the endpoint's `end` is **exclusive** and requires `start < end` (`start===end` → HTTP 500 `startTime must be before endTime`); the +1 also makes same-day calls like `getSchedule(today, today)` valid. Hard 31-day max span per call, counted by calendar date (HTTP 500 beyond). Still filter client-side (third-party dep): `shifts.filter(s => s.date >= from && s.date <= to)`
...
- `/coverage-request`: character needed when `showCharacters(show)` is non-null (MFB/Endings). If omitted, auto-detected from the requester's Discord role via `shows.getShowRole(guild, userId, show)` (same lookup MFB/Endings fill-detection uses) when it resolves to exactly one character; ambiguous (no role, or both roles) falls back to prompting. Explicit `character` option always overrides detection. `customId: coverage_request_modal:{SHOW}:{CHARACTER}`
- **Shift picker (PRD #152, slices #154/#156/#157):** a *linked* requester picks from their own upcoming Bookeo shifts for the chosen show instead of typing them. `execute`: unlinked → explanatory ephemeral reply + "Enter shift manually" button (a reply can't be followed by `showModal`, so the manual path goes through the button); linked → `deferReply` → `bookeo.getScheduleForDays(startDate, PICKER_HORIZON_DAYS)` (90 days) → **multi**-pick select (`coverage_pick:{SHOW}:{CHARACTER}`, `maxValues` = # options, capped at Discord's 25-option limit — soonest 25 shown if more) *plus* the manual button. Empty/Bookeo-error → notice explaining why + manual button (`coverage_manual:{SHOW}:{CHARACTER}` → `showModal`). Both the picker-select and modal paths call the shared `createCoverageRequest(interaction, { show, character, shifts })` (dup-check → post) so behavior is identical; character is fixed per picker (from the customId) and applied to every selected shift. Blank (0-guest) shows are included — verified via `guest_count`. Pure core in `coverage.js`: `buildPickableShifts(scheduleRows, bookeoName, { now, show })` (filter to member case-insensitively, drop past-Central, dedupe, sort, `parseTime`→canonical `HH:MM`), `encodeShiftValue`/`decodeShiftValue` (`YYYY-MM-DD|HH:MM`), and `decodeShiftValues(values)` (multi-select fan-out → array of `{date,time}`, drops malformed). Adding/removing component handlers here does NOT need `deploy-commands` (name/options unchanged).
- **90-day picker horizon:** the bookeo-asst `/api/schedule` endpoint caps each call at a 31-day `start`/`end` span (a wider range returns HTTP 500), so a single `getSchedule` call can't reach a 90-day horizon. `bookeo.getScheduleForDays(from, days)` in `lib/bookeo.js` stitches it from parallel `CHUNK_DAYS`-day-chunk `getSchedule` calls (`CHUNK_DAYS = 30`, safely under the 31-day cap → ~3 calls for 90 days; each still hits the normal 5-min per-call cache) via pure helpers `_buildChunkRanges` (chunk boundaries) + `_mergeScheduleChunks` (dedupe by date+time+show, clip to `[from, to]` — the final chunk can overshoot). `/coverage-request`'s picker and `/member-schedule` both use it (90-day horizon, no `week_of` option — `/member-schedule` shows a member's full upcoming schedule in one reply, clamped to Discord's 2000-char content limit). `/schedule` is unchanged (still single-call, ~7 days).
...
- **Slice 6 (#149) = custom-game takers + games in stats:** new `custom_games.takers` column (JSON `[{ role, userId }]`, ALTER migration). `confirm.js` persists it at confirm time in both paths via `buildGameTakersRecord(...)` → `db.setCustomGameTakers` (single-role: `[{role:null,userId}]`; multi-role: `plan.takers`). Forward-only. `computeCoverageStats(rows, { games, ... })` folds game takers into leaderboard **covers** (never requests) and games into fill-rate: a game is `covered` only when confirmed AND has takers, `unfilledPassed` when unconfirmed AND its date passed; confirmed-without-takers (pre-migration or deactivated) and pending games are excluded (never mislabeled). Games skip the timing and most-needed sections. Data: `repo.getStatsGameRows()` → `db.getCoverageStatsGameRows()` (posted games only). `buildGameTakersRecord` is exported from `coverage-stats.js` and unit-tested.
- All slices #144–#149 shipped. `/coverage-stats` is complete per PRD #143.
...
- **Schedule map (PRD #153):** cron strings live once in `scheduler.CRON`; `scheduler.JOB_META` (key → `{ label, crons[] }`) is the single source for job labels (consumed by `buildJobRegistry`) AND the schedule. `nextCronFire(cronExpr, now)` / `buildJobScheduleView(now)` compute next-fire in Central (DST-aware via `checkin.centralTimeToUnix`). `runTrackedJob(registry, key, notify)` = `runJob(label, () => jobRuns.recordJobRun(key, run), notify)` — records the run AND notifies on failure, no double-handling.

## /bot-status (PRD #153) — owner health command
- **Owner-gated** (`owner.isOwner`, + `ManageGuild` default perms for visibility, like `/coverage-stats`), ephemeral. New slash command → re-run `deploy-commands`.
- **Pure render:** `lib/bot-status.js` `buildStatusEmbed(snapshot, { now })` → plain embed `{ title, color, fields }` (no I/O/clock/Discord). Sections rendered from a snapshot the command assembles: Health, Next scheduled runs, Last run, Data at a glance, Recent errors. Times render as Discord `<t:unix:R>` tags (deterministic). Also exports pure `partitionCastLinks(bookeoNames, linkedNames)` (case-insensitive, de-duped).
- **Job-run instrumentation (shared with dashboard #115):** `lib/job-runs.js` — `recordJobRun(name, asyncFn)` (records start/finish/duration/status/error to `job_runs`, preserves throw/return, feeds `error-buffer` on failure), `getLastRuns()` (latest per job), `getHistory(name?)`. `job_runs(id, job_name, started_at, finished_at, duration_ms, status, error)` (epoch **ms**), indexed `(job_name, started_at)`; startup prune >14d in `index.js`. DB layer: `db.insertJobRun/getLastJobRuns/getJobRunHistory/deleteExpiredJobRuns`.
- **Recent-errors buffer (shared with #115):** `lib/error-buffer.js` — bounded ring buffer (`createErrorBuffer(cap=25)` factory + module-level shared `push`/`list`/`clear`); `list()` newest-first. Fed by `index.js` global handlers (`unhandledRejection`, reaction add/remove catches, top-level command catch) and by `recordJobRun` (tagged `job:<name>`).
- **Bookeo reachability:** `bookeo.probeReachability(from, to)` → `reachable | stale | unreachable`; reuses the 5-min cache (fresh cache → reachable with no call), tracks `_lastSuccessAt` for the stale window. No new upstream call path.
- **Data counts:** via `coverage-repository` (`getOpenShifts/getOpenGames/getUnconfirmedShifts/getUnconfirmedGames`), `checkin.countPendingCheckinsToday(date)`, and `partitionCastLinks(today's Bookeo cast, members.getAllLinkedMembers().map(m=>m.bookeoName))` — no raw `db` from the command.
