...
- **Shift picker (slice #154):** a *linked* requester picks from their own upcoming Bookeo shifts for the chosen show instead of typing them. `execute`: unlinked → manual modal directly (sync link check, no defer before `showModal`); linked → `deferReply` → `bookeo.getSchedule` → single-pick select (`coverage_pick:{SHOW}:{CHARACTER}`). Empty/Bookeo-error → message + "Enter shift manually" button (`coverage_manual:{SHOW}:{CHARACTER}` → `showModal`). Both the picker-select and modal paths call the shared `createCoverageRequest(interaction, { show, character, shifts })` (dup-check → post) so behavior is identical. Blank (0-guest) shows are included — verified via `guest_count`. Pure core in `coverage.js`: `buildPickableShifts(scheduleRows, bookeoName, { now, show })` (filter to member case-insensitively, drop past-Central, dedupe, sort, `parseTime`→canonical `HH:MM`) + `encodeShiftValue`/`decodeShiftValue` (`YYYY-MM-DD|HH:MM`).
