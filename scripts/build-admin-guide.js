...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      spacer(120),
      h2('Coverage Stats (owner only)'),
      p([c('/coverage-stats'), r(' — a running log of who requests coverage and who covers for them. '), b('Owner only'), r(' (hidden from other admins); the report is private and covers your whole history.')]),
      spacer(80),
      bullet([b('Leaderboard'), r(' — per person: shifts + games covered, shifts requested (cancelled shown separately), and give/take ratio.')]),
      bullet([b('Fill rate'), r(' — covered / cancelled / never-filled-and-passed, overall and per show.')]),
      bullet([b('Timing'), r(' — median & average time to coverage (measured to when a cover is confirmed) and lead time before showtime.')]),
      bullet([b('Most-needed'), r(' — which shows, weekdays, and characters generate the most requests.')]),
      spacer(80),
      p([r('Options: '), c('show:'), r(' one show · '), c('person:'), r(' one person’s detail · '), c('since:'), r(' only shifts on/after a date. Custom-game covers are counted from when this feature shipped forward.')]),
      ...
      commandTable([
        ...
        },
        {
          ...
          commands: [
            ...
            ['/coverage-stats',         '(Owner only) Running log of covers, requests, fill rate, timing'],
