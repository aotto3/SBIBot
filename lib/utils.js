...
// Deferred requires to avoid circular deps at module load — resolved on first call
let _cfg   = null;
let _shows = null;
function cfg()   { return _cfg   ?? (_cfg   = require('./config')); }
function shows() { return _shows ?? (_shows = require('./shows'));  }
...
// ─── Coverage reminder formatting ─────────────────────────────────────────────

const MONTH_FULL = ['January','February','March','April','May','June',
                    'July','August','September','October','November','December'];

/**
 * Format a YYYY-MM-DD date and optional HH:MM (24h) time into a human-readable
 * string for coverage reminder messages.
 * Examples: "July 17, 2026 at 7:30pm" | "July 17, 2026" (when timeStr is null)
 */
function formatShiftDateTime(dateStr, timeStr) {
  const [y, mo, d] = dateStr.split('-').map(Number);
  const datePart   = `${MONTH_FULL[mo - 1]} ${d}, ${y}`;
...
function nextOccurrence(meeting, from = null) {
  ...
  if (!from) {
    const centralStr = new Date().toLocaleString('en-US', { timeZone: CENTRAL_TZ });
...
/**
 * Resolve a Discord channel by name from the guild's channel cache.
 * Throws if the channel is not found so callers surface misconfiguration fast.
 *
 * @param {import('discord.js').Guild} guild
 * @param {string} name  Exact channel name (e.g. 'ggb-times')
 */
async function resolveChannelByName(guild, name) {
  const channel = guild.channels.cache.find(c => c.name === name);
  if (!channel) throw new Error(`Channel '${name}' not found in guild`);
  return channel;
}

/**
 * Resolve the coverage-request channel for a show/character combo.
 * Uses a bot_config override ID if set; otherwise resolves '{prefix}-{role}' by name.
 *
 * @param {import('discord.js').Guild} guild
 * @param {string}      showKey
 * @param {string|null} character  Required for multi-role shows (e.g. 'Daphne')
 */
async function resolveCoverageChannel(guild, showKey, character) {
  const overrideId = cfg().getCoverageChannelId(showKey, character);
  if (overrideId) {
    const ch = guild.channels.cache.get(overrideId);
    if (!ch) throw new Error(`Override channel ID ${overrideId} for ${showKey} coverage not found in guild`);
    return ch;
  }
  const slug = character
    ? character.toLowerCase()
    : shows().showAutoRole(showKey)?.toLowerCase();
  if (!slug) throw new Error(`[resolveCoverageChannel] No character or autoRole for show "${showKey}"`);
  return resolveChannelByName(guild, `${shows().showPrefix(showKey)}-${slug}`);
}

/**
 * Resolve the check-in alert channel for a show.
 * Uses a bot_config override ID if set; otherwise resolves '{prefix}-times' by name.
 *
 * @param {import('discord.js').Guild} guild
 * @param {string} showKey
 */
async function resolveCheckinChannel(guild, showKey) {
  const overrideId = cfg().getCheckinAlertChannelId(showKey);
  if (overrideId) {
    const ch = guild.channels.cache.get(overrideId);
    if (!ch) throw new Error(`Override channel ID ${overrideId} for ${showKey} check-in not found in guild`);
    return ch;
  }
  return resolveChannelByName(guild, `${shows().showPrefix(showKey)}-times`);
}

/**
 * Resolve the custom game channel for a show.
 * Uses a bot_config override ID if set; otherwise resolves '{prefix}-times' by name.
 *
 * @param {import('discord.js').Guild} guild
 * @param {string} showKey
 */
async function resolveCustomGameChannel(guild, showKey) {
  const overrideId = cfg().getCustomGameChannelId(showKey);
  if (overrideId) {
    const ch = guild.channels.cache.get(overrideId);
    if (!ch) throw new Error(`Override channel ID ${overrideId} for ${showKey} custom game not found in guild`);
    return ch;
  }
  return resolveChannelByName(guild, `${shows().showPrefix(showKey)}-times`);
}
