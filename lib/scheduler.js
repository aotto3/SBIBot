...
function start(client) {
  ...
  cron.schedule(CRON.MIDNIGHT_0005, async () => {
    ...
    await runJob(registry['checkin-seed'].label, registry['checkin-seed'].run, notify);
  }, { timezone: CENTRAL_TZ });
  ...
  cron.schedule(CRON.DAILY_8AM, async () => {
    ...
    await runJob(registry['meeting-reminders'].label,     registry['meeting-reminders'].run,     notify);
    await runJob(registry['custom-game-reminders'].label, registry['custom-game-reminders'].run, notify);
    await runJob(registry['coverage-pings'].label,        registry['coverage-pings'].run,        notify);
  }, { timezone: CENTRAL_TZ });

  // ── 9pm EOD coverage reminder ─────────────────────────────────────────────
  cron.schedule(CRON.DAILY_9PM, async () => {
    await runJob(registry['eod-reminder'].label, registry['eod-reminder'].run, notify);
  }, { timezone: CENTRAL_TZ });
  ...
  cron.schedule(CRON.SUNDAY_8AM, async () => {
    ...
    await runJob(registry['maybe-nudge'].label, registry['maybe-nudge'].run, notify);
  }, { timezone: CENTRAL_TZ });
  ...
  cron.schedule(CRON.WEEKLY_MON_848AM, async () => {
    ...
    await runJob('Weekly shift DMs', () => runShiftDMs(discord, bkAdapter, 'weekly'), notify);
  }, { timezone: CENTRAL_TZ });
  ...
  cron.schedule(CRON.DAILY_848AM, async () => {
    ...
    await runJob(registry['shift-dms'].label,   registry['shift-dms'].run,   notify);
    await runJob(registry['latebooking'].label, registry['latebooking'].run, notify);
  }, { timezone: CENTRAL_TZ });
