...
const { runJob, notifyJobOutcome } = require('./job-notifier');
const jobRuns = require('./job-runs');

/**
 * Run a registry job with both instrumentation layers: recordJobRun records the
 * run (start/finish/duration/status) keyed on the logical job name, and runJob
 * notifies on failure. Composed so a failing job is recorded AND notified once.
 */
function runTrackedJob(registry, key, notify) {
  const { label, run } = registry[key];
  return runJob(label, () => jobRuns.recordJobRun(key, run), notify);
}

// ─── Cron slots ─────────────────────────────────────────────────────────────
// Named cron expressions, defined once. The wiring in start() and the JOB_META
// map below both reference these so no cron string is written twice (and
// /bot-status's schedule view can't drift from what actually fires).
const CRON = {
  MIDNIGHT_0005:    '5 0 * * *',   // 12:05am daily — check-in re-seed
  DAILY_8AM:        '0 8 * * *',   // 8:00am daily
  SUNDAY_8AM:       '0 8 * * 0',   // 8:00am Sundays — maybe-nudge
  WEEKLY_MON_848AM: '48 8 * * 1',  // 8:48am Mondays — weekly shift DMs
  DAILY_848AM:      '48 8 * * *',  // 8:48am daily — daily shift DMs + late-booking
  DAILY_9PM:        '0 21 * * *',  // 9:00pm daily — EOD coverage reminder
};

/**
 * Logical scheduled jobs → human label + the cron slot(s) each fires on.
 * Single source of truth for job names/labels (used by buildJobRegistry, the
 * failure notifier, and /rerun-job) and their schedule (used by /bot-status).
 * `crons` lists every slot a job runs in — check-in seeding fires at midnight
 * and again at 8am, so its next-fire is the sooner of the two.
 */
const JOB_META = {
  'meeting-reminders':     { label: 'Meeting reminders',     crons: [CRON.DAILY_8AM] },
  'coverage-pings':        { label: 'Coverage role pings',   crons: [CRON.DAILY_8AM] },
  'custom-game-reminders': { label: 'Custom-game reminders', crons: [CRON.DAILY_8AM] },
  'eod-reminder':          { label: 'EOD coverage reminder', crons: [CRON.DAILY_9PM] },
  'maybe-nudge':           { label: 'Weekly maybe-nudge',    crons: [CRON.SUNDAY_8AM] },
  'shift-dms':             { label: 'Daily shift DMs',       crons: [CRON.DAILY_848AM] },
  'latebooking':           { label: 'Late-booking seed',     crons: [CRON.DAILY_848AM] },
  'checkin-seed':          { label: 'Check-in seeding',      crons: [CRON.MIDNIGHT_0005, CRON.DAILY_8AM] },
};

/**
 * Registry of scheduled jobs: key → { label, run }. Single source of truth shared
 * by the cron wiring (below), the failure notifier (job names), and /rerun-job.
 * `run` closes over the injected deps so callers just invoke it.
 */
function buildJobRegistry({ discord, repo, bkAdapter, client, notify = null }) {
  const L = key => JOB_META[key].label;
  return {
    'meeting-reminders':     { label: L('meeting-reminders'),     run: () => runMeetingReminderCheck(discord) },
    'coverage-pings':        { label: L('coverage-pings'),        run: () => runCoverageRolePings(discord, repo) },
    'custom-game-reminders': { label: L('custom-game-reminders'), run: () => runCustomGameReminders(discord) },
    'eod-reminder':          { label: L('eod-reminder'),          run: () => runEodCoverageReminder(discord, repo) },
    'maybe-nudge':           { label: L('maybe-nudge'),           run: () => runMaybeNudge(discord, repo) },
    'shift-dms':             { label: L('shift-dms'),             run: () => runShiftDMs(discord, bkAdapter, 'daily') },
    'latebooking':           { label: L('latebooking'),           run: () => runLatebookingSeed(discord, bkAdapter, notify) },
    'checkin-seed':          { label: L('checkin-seed'),          run: () => checkin.seedWithRetry(client, utils.todayCentral(), 1) },
  };
}

/**
 * Next fire time (epoch ms) for a simple cron expression, in Central time.
 * Supports the subset our schedule uses: a fixed minute and hour, `*` for
 * day-of-month and month, and either `*` or a single numeric day-of-week
 * (0 = Sunday). Pure — inject `now` for deterministic tests.
 *
 * @param {string} cronExpr  e.g. "0 8 * * *" or "0 8 * * 0"
 * @param {number|Date} [now]
 * @returns {number|null}  epoch ms of the next fire, or null if unparseable
 */
function nextCronFire(cronExpr, now = new Date()) {
  ...
  const dow    = fields[4] === '*' ? null : Number(fields[4]);
  if (!Number.isInteger(minute) || !Number.isInteger(hour)) return null;

  // Central calendar date of `now`, then walk forward day by day.
  const central = new Intl.DateTimeFormat('en-CA', {
    timeZone: CENTRAL_TZ, year: 'numeric', month: '2-digit', day: '2-digit',
  }).format(new Date(nowMs));
  const [y, mo, d] = central.split('-').map(Number);

  for (let offset = 0; offset <= 8; offset++) {
    const cand = new Date(Date.UTC(y, mo - 1, d + offset)); // pure date arithmetic
    if (dow !== null && cand.getUTCDay() !== dow) continue;
    const dateStr = `${cand.getUTCFullYear()}-`
      + `${String(cand.getUTCMonth() + 1).padStart(2, '0')}-`
      + `${String(cand.getUTCDate()).padStart(2, '0')}`;
    const fireMs = checkin.centralTimeToUnix(dateStr, hour, minute) * 1000;
...
/**
 * The schedule view for /bot-status: each logical job with its next fire time.
 * Reads JOB_META (the single schedule source) so it can't drift from the wiring.
 *
 * @param {number|Date} [now]
 * @returns {Array<{ key: string, label: string, nextRun: number|null }>}
 */
function buildJobScheduleView(now = new Date()) {
  return Object.entries(JOB_META).map(([key, meta]) => {
    const times = meta.crons.map(c => nextCronFire(c, now)).filter(t => t != null);
    return { key, label: meta.label, nextRun: times.length ? Math.min(...times) : null };
  });
}
...
function start(client) {
  ...
  // Notify the ops contact (DM) + error channel whenever a job throws or drops items.
  const notify = (outcome) => notifyJobOutcome(discord, outcome, {
    opsContactId:   cfg.getOpsContactId(),
    errorChannelId: cfg.getErrorChannelId(),
  });

  const registry  = buildJobRegistry({ discord, repo, bkAdapter, client, notify });
  ...
  cron.schedule(CRON.MIDNIGHT_0005, async () => {
    console.log(`[checkin] Midnight re-seeding for ${utils.todayCentral()}`);
    await runTrackedJob(registry, 'checkin-seed', notify);
  }, { timezone: CENTRAL_TZ });

  // ── Daily 8am checks ───────────────────────────────────────────────────────
  // • Meeting reminders (7d and 24h windows)
  // • Unfilled custom game reminders (>48h old, not yet filled)
  cron.schedule(CRON.DAILY_8AM, async () => {
    console.log('[scheduler] Running daily checks');
    await runTrackedJob(registry, 'meeting-reminders',     notify);
    await runTrackedJob(registry, 'custom-game-reminders', notify);
    await runTrackedJob(registry, 'coverage-pings',        notify);
  }, { timezone: CENTRAL_TZ });

  // ── 9pm EOD coverage reminder ─────────────────────────────────────────────
  cron.schedule(CRON.DAILY_9PM, async () => {
    await runTrackedJob(registry, 'eod-reminder', notify);
  }, { timezone: CENTRAL_TZ });

  // ── Weekly maybe-nudge — every Sunday at 8am ──────────────────────────────
  // DMs anyone still marked "maybe" on an open, uncovered coverage shift/game.
  cron.schedule(CRON.SUNDAY_8AM, async () => {
    console.log('[scheduler] Running weekly maybe-nudge job');
    await runTrackedJob(registry, 'maybe-nudge', notify);
  }, { timezone: CENTRAL_TZ });
  ...
  cron.schedule(CRON.DAILY_8AM, async () => {
    console.log(`[checkin] 8am backup re-seeding for ${utils.todayCentral()}`);
    await runTrackedJob(registry, 'checkin-seed', notify);
  }, { timezone: CENTRAL_TZ });

  // ── Weekly shift DMs — every Monday at 8:48am ────────────────────────────
  // Runs before 9am show starts so Bookeo still returns cast member info.
  cron.schedule(CRON.WEEKLY_MON_848AM, async () => {
    if (!cfg.isWeeklyShiftsEnabled()) return;
    console.log('[scheduler] Running weekly shift DM job');
    // Same logical job as the daily shift DMs (different window) — record under 'shift-dms'.
    await runJob('Weekly shift DMs', () => jobRuns.recordJobRun('shift-dms', () => runShiftDMs(discord, bkAdapter, 'weekly')), notify);
  }, { timezone: CENTRAL_TZ });

  // ── Daily 24hr shift DMs — every day at 8:48am ───────────────────────────
  // Runs before 9am show starts so Bookeo still returns cast member info.
  // After DMs, seeds the late-booking baseline for today's blank shows.
  cron.schedule(CRON.DAILY_848AM, async () => {
    if (!cfg.isDailyShiftsEnabled()) return;
    console.log('[scheduler] Running daily 24hr shift DM job');
    await runTrackedJob(registry, 'shift-dms',   notify);
    await runTrackedJob(registry, 'latebooking', notify);
  }, { timezone: CENTRAL_TZ });
  ...
  if (todayLatebooking.length) {
    ...
    for (const row of todayLatebooking) {
      _scheduleLatebookingCheck(discord, bkAdapter, row, notify);
    }
  }
...
/**
 * Return one-time meetings whose date is strictly before today.
 * Pure — no I/O.
 *
 * @param {object[]} meetings  Active meeting rows from DB
 * @param {Date}     today     The reference date
 * @returns {object[]}  Subset of meetings to hard-delete
 */
function planExpiredMeetings(meetings, today) {
  return meetings.filter(m => {
    if (m.recurrence_type) return false;
 ...
 *
 * Monthly recurring meetings (`monthly_weekday`) use a catch-up window: a
 * reminder fires when its occurrence is within range (≤7 days for '7d', ≤1 day
 * for '24h') AND that reminder type hasn't already been recorded for the
 * occurrence. This self-heals a missed 8am run (redeploy/crash) instead of
 * permanently skipping the month, and covers a first occurrence that is already
 * <7 days away when the meeting is scheduled. Weekly and one-time meetings keep
 * the original exact-day behavior.
 *
 * @param {object[]} meetings  Active meeting rows from DB
 * @param {Date}     today     The reference date (use todayCentral()-derived Date in prod)
 * @param {(meetingId: number, instanceDate: string, window: '7d'|'24h') => boolean} [hasSent]
 *        Predicate reporting whether a reminder was already recorded for an occurrence.
 *        Defaults to "nothing sent" so the pure function is easy to test.
 * @returns {Array<{ meeting: object, dateStr: string, window: '7d'|'24h' }>}
 */
function planMeetingReminders(meetings, today, hasSent = () => false) {
  const result = [];

  for (const meeting of meetings) {
    if (meeting.recurrence_type === 'monthly_weekday') {
      const occ = utils.nextOccurrence(meeting, today);
      if (!occ) continue;
      const occStr    = utils.toDateString(occ);
      const daysUntil = Math.round((occ.getTime() - today.getTime()) / 86400000);

      if (meeting.reminder_7d && daysUntil <= 7 && !hasSent(meeting.id, occStr, '7d')) {
        result.push({ meeting, dateStr: occStr, window: '7d' });
      }
      if (meeting.reminder_24h && daysUntil <= 1 && !hasSent(meeting.id, occStr, '24h')) {
        result.push({ meeting, dateStr: occStr, window: '24h' });
      }
      continue;
    }

    // Weekly + one-time: exact-day behavior (unchanged).
...
async function runMeetingReminderCheck(discord) {
  ...
  const expired = planExpiredMeetings(meetings, today);
  for (const meeting of expired) {
    db.deleteMeeting(meeting.id);
    console.log(`[meetings] Auto-deleted expired one-time meeting ${meeting.id} "${meeting.title}"`);
  }

  const active  = meetings.filter(m => !expired.some(e => e.id === m.id));
  const planned = planMeetingReminders(active, today,
    (meetingId, instanceDate, window) => db.hasReminderBeenSent(meetingId, instanceDate, window));
...
async function runShiftDMs(discord, bookeoAdapter, mode) {
  ...
  try {
    ...
    return { planned: 0, sent: 0, failed: [] };
  }

  // Defensively filter to only shifts within the requested window: the range is
  // scoped upstream via start/end, but bookeo-asst is a third-party dependency,
  // so we don't rely on it blindly.
  ...
  for (const { discord_id, castName, dmText } of planned) {
    ...
    try {
      ...
      skipped++;
      failed.push({ id: discord_id, type: 'shift-dm', reason: err.message });
    }
  }

  console.log(`[scheduler] Shift DM run complete — sent: ${sent}, no link: ${skipped}`);
  return { planned: planned.length, sent, failed };
}
...
async function runCustomGameReminders(discord) {
  ...
  if (!unfilled.length) return { planned: 0, sent: 0, failed: [] };

  const planned     = planCustomGameReminders(unfilled);
  if (!planned.length) return { planned: 0, sent: 0, failed: [] };
  ...
  // Hydrate each guild's roles + members once per run — the member roster fetch is a
  // rate-limited gateway opcode-8 request, so fetching per game floods the limit.
  ...
  for (const { game, dateTimeStr } of planned) {
    try {
      ...
      if (game.message_id) {
        try {
          if (!hydratedGuilds.has(guild.id)) {
            await discord.fetchGuildRoles(guild);
            await fetchGuildMembersWithRetry(discord, guild);
      ...
      const link = game.message_id
        ? `https://discord.com/channels/${guild.id}/${game.channel_id}/${game.message_id}`
        : null;
      const respondLine = link
        ? `Respond if you have not yet: ${link}`
        : `Respond if you have not yet.`;
      const content = `<@${game.requester_id}>, ${pingStr} Heads up - we are still looking for coverage for **${showLabel(game.show)}** for ${dateTimeStr}. ${respondLine}`;
      ...
      failed.push({ id: game.id, type: 'custom-game', reason: err.message });
    }
  }
 ...
 */
async function runLatebookingSeed(discord, bookeoAdapter, notify = null) {
  ...
  // Defensively filter to today only (bookeo-asst is a third-party dependency).
  ...
  for (const row of rows) {
    _scheduleLatebookingCheck(discord, bookeoAdapter, row, notify);
  }
}
 ...
 */
function _scheduleLatebookingCheck(discord, bookeoAdapter, row, notify = null) {
  ...
  if (delayMs > 0) {
    ...
    setTimeout(() => runLatebookingCheck(discord, bookeoAdapter, row.date, notify), delayMs);
  } else {
    ...
    runLatebookingCheck(discord, bookeoAdapter, row.date, notify);
  }
}
 ...
 */
async function runLatebookingCheck(discord, bookeoAdapter, date, notify = null) {
  ...
  for (const row of newlyBooked) {
    ...
    for (const bookeoName of row.cast) {
      ...
      try {
        ...
        failed.push({ id: bookeoName, type: 'latebooking-dm', reason: err.message });
      }
    }
  ...
  if (failed.length && notify) {
    try {
      await notify({ label: 'Late-booking alerts', planned, sent, failed });
    } catch (err) {
      console.error('[latebooking] Failed to send partial-failure notice:', err.message);
    }
  }

  return { planned, sent, failed };
}
