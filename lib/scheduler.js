 ...
 */
function buildJobRegistry({ discord, repo, bkAdapter, client }) {
  return {
    ...
    'latebooking':           { label: 'Late-booking seed',     run: () => runLatebookingSeed(discord, bkAdapter) },
...
function start(client) {
  ...
  // Notify the ops contact (DM) + error channel whenever a job throws. Ops contact
  // defaults to the owner id until /set-ops-contact configures one (slice #135).
  const notify = (outcome) => notifyJobOutcome(discord, outcome, { errorChannelId: cfg.getErrorChannelId() });
