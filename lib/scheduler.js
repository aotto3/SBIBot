...
const { showLabel, showAutoRole, showCharacters, getDiscordRoleName, getShowRole } = require('./shows');
const { planMissingRolePings, buildEodDM, analyzeCoverage } = require('./coverage');
      ...
      if (game.message_id) {
        try {
          ...
          for (const reaction of msg.reactions.cache.values()) {
            ...
            users.filter(u => !u.bot).forEach(u => reactorIds.add(u.id));
          }

          // Determine which roles still need coverage
          let roleNamesToTarget = [];
          const yesUsers = await fetchYesReactors(msg);
...
async function runCoverageRolePings(client) {
  ...
  // Maps: messageId → show / dateTimeStr (needed in the send loop)
  ...
  for (const { channelId, roleNames, messageId } of plan) {
    try {
      ...
      // Collect all non-bot reactors (any emoji) so we know who has responded
      const reactorIds = new Set();
      try {
