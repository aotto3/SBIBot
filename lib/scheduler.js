...
function buildJobRegistry({ discord, repo, bkAdapter, client, notify = null }) {
  return {
    'meeting-reminders':     { label: 'Meeting reminders',     run: () => runMeetingReminderCheck(discord) },
    'coverage-pings':        { label: 'Coverage role pings',   run: () => runCoverageRolePings(discord, repo) },
    'custom-game-reminders': { label: 'Custom-game reminders', run: () => runCustomGameReminders(discord) },
    'eod-reminder':          { label: 'EOD coverage reminder', run: () => runEodCoverageReminder(discord, repo) },
    'maybe-nudge':           { label: 'Weekly maybe-nudge',    run: () => runMaybeNudge(discord, repo) },
    'shift-dms':             { label: 'Daily shift DMs',       run: () => runShiftDMs(discord, bkAdapter, 'daily') },
    'latebooking':           { label: 'Late-booking seed',     run: () => runLatebookingSeed(discord, bkAdapter, notify) },
    'checkin-seed':          { label: 'Check-in seeding',      run: () => checkin.seedWithRetry(client, utils.todayCentral(), 1) },
  };
}
