...
function makeDiscordAdapter(client) {
  return {
    ...
    removeReaction(reaction, userId) {
      return reaction.users.remove(userId);
    },

    /**
     * Live Discord connection status for the dashboard health page.
     * Kept on the adapter so the web layer never touches the raw client.
     *
     * @returns {{ ready: boolean, wsPing: number, guildCount: number, userTag: string|null }}
     */
    getStatus() {
      return {
