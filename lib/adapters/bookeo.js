...
function makeBookeoAdapter() {
  return {
     ...
     *
     * Note: bookeo.getSchedule scopes the range via `start`/`end` on the wire,
     * but the endpoint is a third-party dependency, so callers still filter the
     * results client-side to the requested window as a defensive measure.
     *
