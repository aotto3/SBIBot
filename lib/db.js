...
db.exec(`
  ...
  -- Shared per-occurrence RSVP for monthly recurring meetings. A user has one
  -- RSVP per occurrence but may react on either the 7d or 24h post; the post
  -- column records where their current reaction physically lives so a stale
  -- reaction can be cleaned off the other post (most-recent-wins).
  CREATE TABLE IF NOT EXISTS meeting_rsvps (
    meeting_id     INTEGER NOT NULL REFERENCES meetings(id),
    instance_date  TEXT NOT NULL,   -- YYYY-MM-DD of the specific occurrence
    user_id        TEXT NOT NULL,   -- Discord user ID
    status         TEXT NOT NULL,   -- 'yes' | 'maybe' | 'no'
    post           TEXT NOT NULL,   -- '7d' | '24h' — which post the reaction sits on
    display_name   TEXT,            -- resolved name for tracker display (restart-safe)
    updated_at     INTEGER NOT NULL,
    PRIMARY KEY (meeting_id, instance_date, user_id)
  );
  ...
  CREATE TABLE IF NOT EXISTS coverage_ping_messages (
    ping_message_id      TEXT PRIMARY KEY,
  ...
  CREATE TABLE IF NOT EXISTS job_runs (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    job_name     TEXT NOT NULL,          -- logical job key (e.g. 'coverage-pings')
...
try { db.exec('ALTER TABLE custom_games ADD COLUMN takers TEXT'); } catch {}
try { db.exec('ALTER TABLE meeting_rsvps ADD COLUMN display_name TEXT'); } catch {}
...
function deleteMeeting(id) {
  // node:sqlite has no better-sqlite3-style db.transaction(); drive the
  // transaction explicitly. Delete children first (foreign_keys = ON).
  db.exec('BEGIN');
  try {
    db.prepare('DELETE FROM meeting_members WHERE meeting_id = ?').run(id);
    db.prepare('DELETE FROM meeting_reminders_sent WHERE meeting_id = ?').run(id);
...
// ─── Meeting RSVPs (shared per-occurrence state, monthly recurring) ────────────

function setMeetingRsvp(meetingId, instanceDate, userId, status, post, updatedAt, displayName = null) {
  db.prepare(`
    INSERT INTO meeting_rsvps (meeting_id, instance_date, user_id, status, post, display_name, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT (meeting_id, instance_date, user_id)
    DO UPDATE SET status = excluded.status, post = excluded.post,
                  display_name = excluded.display_name, updated_at = excluded.updated_at
  `).run(meetingId, instanceDate, userId, status, post, displayName, updatedAt);
}

function removeMeetingRsvp(meetingId, instanceDate, userId) {
  db.prepare(`
    DELETE FROM meeting_rsvps
    WHERE meeting_id = ? AND instance_date = ? AND user_id = ?
  `).run(meetingId, instanceDate, userId);
}

function getMeetingRsvp(meetingId, instanceDate, userId) {
  return db.prepare(`
    SELECT * FROM meeting_rsvps
    WHERE meeting_id = ? AND instance_date = ? AND user_id = ?
  `).get(meetingId, instanceDate, userId) ?? null;
}

function getMeetingRsvps(meetingId, instanceDate) {
  return db.prepare(`
    SELECT * FROM meeting_rsvps
    WHERE meeting_id = ? AND instance_date = ?
  `).all(meetingId, instanceDate);
}

/** Return the 7d and 24h Discord message IDs for an occurrence, or null each. */
function getMeetingPostIds(meetingId, instanceDate) {
  const rows = db.prepare(`
    SELECT reminder_type, message_id FROM meeting_reminders_sent
    WHERE meeting_id = ? AND instance_date = ? AND message_id IS NOT NULL
  `).all(meetingId, instanceDate);
  const byType = {};
  for (const r of rows) byType[r.reminder_type] = r.message_id;
  return { '7d': byType['7d'] ?? null, '24h': byType['24h'] ?? null };
}
...
/**
 * Outstanding coverage shifts for /list-outstanding-requests:
 * open shifts dated today-or-later, joined with request info.
 * Optional `show` narrows to a single show.
 */
function getOutstandingCoverageShifts(today, show = null) {
  const params = [today];
  let sql = `
    SELECT cs.id, cs.date, cs.time, cs.shift_message_id, cs.fillable_notified,
           cr.show, cr.character, cr.channel_id, cr.requester_name, cr.requester_id,
           cr.header_message_id
    FROM coverage_shifts cs
    JOIN coverage_requests cr ON cs.request_id = cr.id
    WHERE cs.status = 'open' AND cs.date >= ?
  `;
  if (show) { sql += ' AND cr.show = ?'; params.push(show); }
  sql += ' ORDER BY cs.date, cs.time';
  return db.prepare(sql).all(...params);
}

/**
 * All coverage shifts joined with their request info, across every status
 * (open/covered/cancelled). Used by /coverage-stats to build the leaderboard,
 * timing, most-needed, and fill-rate sections. Fully retroactive — no filters.
 */
function getCoverageStatsShiftRows() {
  return db.prepare(`
    SELECT cs.id, cs.request_id, cs.date, cs.time, cs.status,
           cs.confirmed_taker_id, cs.confirmed_at,
           cr.show, cr.character, cr.requester_id, cr.requester_name,
...
/**
 * All posted custom games with their taker record — used by /coverage-stats to
 * fold game covers into the leaderboard and games into the fill-rate section.
 * `takers` is a JSON string (or null for games confirmed before taker logging).
 */
function getCoverageStatsGameRows() {
  return db.prepare(`
    SELECT id, show, date, time, confirmed_at, filled_at, takers
    FROM custom_games
    WHERE message_id IS NOT NULL
  `).all();
}

/** Persist who took a custom game (array of { role, userId }, JSON-encoded). */
function setCustomGameTakers(id, record) {
  db.prepare('UPDATE custom_games SET takers = ? WHERE id = ?').run(JSON.stringify(record), id);
}
...
/**
 * Outstanding custom games for /list-outstanding-requests:
 * unconfirmed, posted games dated today-or-later.
 * `filled_at` distinguishes unfilled (null) from filled-awaiting-confirmation (set).
 * Optional `show` narrows to a single show.
 */
function getOutstandingCustomGames(today, show = null) {
  const params = [today];
  let sql = `
    SELECT id, show, date, time, filled_at, requester_id, channel_id, message_id
    FROM custom_games
    WHERE confirmed_at IS NULL AND message_id IS NOT NULL AND date >= ?
  `;
  if (show) { sql += ' AND show = ?'; params.push(show); }
...
function saveCoveragePingMessage(pingMsgId, originalMsgId, channelId, show, date, time) {
  db.prepare(`
    INSERT OR IGNORE INTO coverage_ping_messages
      (ping_message_id, original_message_id, channel_id, show, date, time)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(pingMsgId, originalMsgId, channelId, show, date, time);
}

function getCoveragePingMessageByPingId(pingMsgId) {
  return db.prepare(
    'SELECT * FROM coverage_ping_messages WHERE ping_message_id = ?'
  ).get(pingMsgId) ?? null;
}

/** Distinct original (coverage post) message ids that were pinged at/after `sinceUnix`. */
function getCoveragePingedMessageIdsSince(sinceUnix) {
  return db.prepare(
    'SELECT DISTINCT original_message_id FROM coverage_ping_messages WHERE created_at >= ?'
  ).all(sinceUnix).map(r => r.original_message_id);
}

function markCoveragePingRedirected(pingMsgId) {
  db.prepare(
    'UPDATE coverage_ping_messages SET redirected = 1 WHERE ping_message_id = ?'
  ).run(pingMsgId);
}

function deleteExpiredCoveragePingMessages(maxAgeSeconds) {
  const cutoff = Math.floor(Date.now() / 1000) - maxAgeSeconds;
  db.prepare('DELETE FROM coverage_ping_messages WHERE created_at < ?').run(cutoff);
}
...
// ─── Job runs (observability — /bot-status, dashboard #115) ───────────────────
// Timestamps are epoch ms (duration_ms needs sub-second precision).

function insertJobRun({ job_name, started_at, finished_at = null, duration_ms = null, status, error = null }) {
  const info = db.prepare(`
    INSERT INTO job_runs (job_name, started_at, finished_at, duration_ms, status, error)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(job_name, started_at, finished_at, duration_ms, status, error);
  return Number(info.lastInsertRowid);
}

/** Latest run per job_name. Relies on SQLite's bare-column-follows-MAX behavior. */
function getLastJobRuns() {
  return db.prepare(`
    SELECT job_name, MAX(started_at) AS started_at, id, finished_at, duration_ms, status, error
    FROM job_runs
    GROUP BY job_name
  `).all();
}

/** Run history newest-first, optionally for one job. */
function getJobRunHistory(jobName = null, limit = 20) {
  if (jobName) {
    return db.prepare(
      'SELECT * FROM job_runs WHERE job_name = ? ORDER BY started_at DESC LIMIT ?'
    ).all(jobName, limit);
  }
  return db.prepare('SELECT * FROM job_runs ORDER BY started_at DESC LIMIT ?').all(limit);
}

function deleteExpiredJobRuns(maxAgeSeconds) {
...
module.exports = {
  ...
  getCoverageStatsShiftRows,
  getCoverageStatsGameRows,
  setCustomGameTakers,
  getOpenCustomGamesForPings,
  getOutstandingCoverageShifts,
  getOutstandingCustomGames,
  ...
  saveCoveragePingMessage,
  getCoveragePingMessageByPingId,
  getCoveragePingedMessageIdsSince,
  markCoveragePingRedirected,
  deleteExpiredCoveragePingMessages,
  insertJobRun,
  getLastJobRuns,
  getJobRunHistory,
