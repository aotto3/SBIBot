...
db.exec(`
  ...
  -- Admin-dashboard audit trail: one row per operator action (login, config
  -- change, link edit, job trigger, …). params/result hold JSON text; message_id
  -- records any Discord message an action produced (v2 undo/delete-last uses it).
  CREATE TABLE IF NOT EXISTS audit_log (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    action      TEXT NOT NULL,
    params      TEXT,             -- JSON
    result      TEXT,             -- JSON or plain string
    message_id  TEXT,             -- Discord message ID, when the action sent one
    created_at  INTEGER NOT NULL DEFAULT (unixepoch())
  );
...
// params/result are stored as TEXT; lib/audit-log.js JSON-serializes structured
// values before calling in and parses them back on read.

function insertAuditLog({ action, params = null, result = null, messageId = null }) {
  const info = db.prepare(
    'INSERT INTO audit_log (action, params, result, message_id) VALUES (?, ?, ?, ?)'
  ).run(action, params, result, messageId);
