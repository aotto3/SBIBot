...
function setMeetingRsvp(meetingId, instanceDate, userId, status, post, updatedAt) {
  db.prepare(`
    INSERT INTO meeting_rsvps (meeting_id, instance_date, user_id, status, post, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT (meeting_id, instance_date, user_id)
    DO UPDATE SET status = excluded.status, post = excluded.post, updated_at = excluded.updated_at
  `).run(meetingId, instanceDate, userId, status, post, updatedAt);
}
