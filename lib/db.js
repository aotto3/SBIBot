...
function createCoverageRequest({ requester_id, requester_name, show, channel_id }) {
  const result = db.prepare(`
    INSERT INTO coverage_requests (requester_id, requester_name, show, channel_id)
    VALUES (?, ?, ?, ?)
  `).run(requester_id, requester_name, show, channel_id);
  return result.lastInsertRowid;
}
...
function getOpenShiftByShowAndDateTime(show, date, time) {
  return db.prepare(`
    SELECT cs.* FROM coverage_shifts cs
    JOIN coverage_requests cr ON cs.request_id = cr.id
