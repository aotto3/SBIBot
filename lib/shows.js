...
/**
 * Return the config object for showKey, or throw a descriptive error.
 * Used by all accessors where an unknown key is a programming error
 * (i.e. a show was added to bookeo.js but not to SHOWS).
 *
 * @param {string} showKey
 * @returns {object} The SHOWS entry
 */
function _requireShow(showKey) {
  const config = SHOWS[showKey];
  if (!config) {
    throw new Error(
      `[shows] Unknown show key "${showKey}". ` +
      `Valid keys: ${Object.keys(SHOWS).join(', ')}. ` +
      `Did you add a show to bookeo.js but forget to add it to SHOWS in shows.js?`
    );
  }
  return config;
}
...
/**
 * Validate that every entry in the show registry has the fields all accessors
 * assume exist. Throws on the first violation so a misconfigured show fails
 * fast at startup rather than silently at the first runtime call.
 *
 * Accepts an optional `shows` param so tests can pass a modified config object.
 * Called once at module load with the default SHOWS.
 *
 * @param {object} [shows=SHOWS]
 */
function validateShows(shows = SHOWS) {
  for (const [key, config] of Object.entries(shows)) {
    if (!config.label) {
      throw new Error(`[shows] SHOWS["${key}"] is missing required field "label"`);
    }
    if (!config.prefix) {
      throw new Error(`[shows] SHOWS["${key}"] is missing required field "prefix"`);
    }
    if (!config.emojis) {
      throw new Error(`[shows] SHOWS["${key}"] is missing required field "emojis"`);
    }
    for (const group of ['yes', 'maybe', 'no']) {
      if (!Array.isArray(config.emojis[group])) {
        throw new Error(`[shows] SHOWS["${key}"].emojis.${group} must be an array`);
      }
    }
    if (config.checkin !== undefined) {
      if (!Array.isArray(config.checkin.roles) || config.checkin.roles.length === 0) {
        throw new Error(`[shows] SHOWS["${key}"].checkin.roles must be a non-empty array`);
      }
      if (typeof config.checkin.callTimeOffset !== 'number') {
        throw new Error(`[shows] SHOWS["${key}"].checkin.callTimeOffset must be a number`);
      }
    }
  }
}

validateShows(); // fail fast at require() time — not at 9am
...
function allEmojisForShow(showKey) {
  const { emojis } = _requireShow(showKey);
...
/**
 * Space-joined display string of every possible reaction emoji for a show,
 * in reaction order (yes, then maybe, then no). Unicode emojis pass through
 * as-is; custom emojis are resolved against the guild's emoji cache.
 * Used to spell out the valid reactions in coverage reminder messages.
 *
 * @param {import('discord.js').Guild} guild
 * @param {string} showKey
 */
function showReactionList(guild, showKey) {
  return allEmojisForShow(showKey).map(e => emojiDisplay(guild, e)).join(' ');
}
...
async function getShowRole(guild, userId, showKey) {
  const config = _requireShow(showKey);
...
function showLabel(showKey) {
  return _requireShow(showKey).label;
}

/** Channel name prefix for a show key (e.g. 'ggb', 'mfb'). */
function showPrefix(showKey) {
  return _requireShow(showKey).prefix;
}
...
function checkinConfig(showKey) {
  return _requireShow(showKey).checkin ?? null;
}
...
function showRoleGroups(showKey) {
  return _requireShow(showKey).roleGroups ?? null;
}
