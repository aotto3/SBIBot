...
/** All shifts (every status) joined with request info — used by /coverage-stats. */
const getStatsShiftRows = () => db.getCoverageStatsShiftRows();

/** All posted custom games with their taker record — used by /coverage-stats. */
const getStatsGameRows = () => db.getCoverageStatsGameRows();

/** Outstanding (open, future-dated) shifts with request info — used by /list-outstanding-requests. */
const getOutstandingShifts = (today, show = null) => db.getOutstandingCoverageShifts(today, show);
...
const setGameTakers        = (id, record) => db.setCustomGameTakers(id, record);
...
/** Outstanding (unconfirmed, posted, future-dated) games — used by /list-outstanding-requests. */
const getOutstandingGames = (today, show = null) => db.getOutstandingCustomGames(today, show);
...
const savePingMessage     = (pingMsgId, originalMsgId, channelId, show, date, time) =>
  db.saveCoveragePingMessage(pingMsgId, originalMsgId, channelId, show, date, time);
const getPingByMessageId  = pingMsgId => db.getCoveragePingMessageByPingId(pingMsgId);
const getPingedMessageIdsSince = since => db.getCoveragePingedMessageIdsSince(since);
const markPingRedirected  = pingMsgId => db.markCoveragePingRedirected(pingMsgId);
