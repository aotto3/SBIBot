...
const { buildConfirmationMessage, buildResolvedHeaderPost, buildShiftPost, planShiftCancel } = require('./coverage');
const { showCharacters, showLabel, getShowRole, getDiscordRoleName } = require('./shows');
const { setMultiRoleSelection, getMultiRoleSelections, clearMultiRoleSelections, planMultiRoleConfirm } = require('./coverage-session');
const { buildGameTakersRecord } = require('./coverage-stats');
...
async function handleMultiRoleSelect(interaction) {
  ...
  setMultiRoleSelection(interaction.user.id, gameId, roleName, takerId);
...
async function handleMultiRoleSubmit(interaction) {
  ...
  const characters = showCharacters(game.show);
  const pending    = getMultiRoleSelections(interaction.user.id, gameId);

  if (pending === undefined) {
    return interaction.reply({
      content: '❌ Your confirmation session has expired (30 min limit). Please reopen the confirmation to start over.',
      flags: MessageFlags.Ephemeral,
    });
  }

  const plan = planMultiRoleConfirm(pending, characters);

  if (!plan.valid) {
    return interaction.update({
      content: `❌ Please select someone for: ${plan.missingRoles.join(', ')}`,
      components: interaction.message.components,
    });
  }

  clearMultiRoleSelections(interaction.user.id, gameId);
  db.confirmCustomGame(gameId);
  db.setCustomGameTakers(gameId, buildGameTakersRecord(plan.takers));
...
async function handleConfirmCoverageSelect(interaction) {
  ...
  if (type === 'shift') {
    ...
    db.setCustomGameTakers(id, buildGameTakersRecord([{ role: null, userId: takerId }]));
  }
  ...
  if (type === 'shift') {
    ...
    if (plan.action === 'delete-all') {
      ...
      if (shift.shift_message_id === request.header_message_id) {
        // Single combined post — update to resolved state
        try {
        ...
        // Separate shift post — mark it cancelled, then resolve the header
      ...
      // This shift's post IS the header — edit to show header for remaining shifts + cancelled note
      try {
        const msg = await channel.messages.fetch(shift.shift_message_id);
      ...
      // Non-header shift, others remain — mark it cancelled
      if (shift.shift_message_id) {
        try {
          const msg = await channel.messages.fetch(shift.shift_message_id);
          await msg.edit({ content: `❌ **Cancelled** — ${buildShiftPost(request, shift)}`, components: [] });
    ...
    await interaction.editReply(`✅ Shift \`${id}\` cancelled.`);
  } else {
    const game = db.getCustomGameById(id);
    ...
    if (game.channel_id && game.message_id) {
      try {
        ...
        console.error(`[confirm] Failed to update game ${id} post:`, err.message);
      }
    }
    console.log(`[confirm] ${interaction.user.tag} cancelled custom game ${id} via /open-coverage`);
    await interaction.editReply(`✅ Custom game \`${id}\` cancelled.`);
  }
}
