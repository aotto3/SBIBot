...
const { buildConfirmationMessage, buildResolvedHeaderPost } = require('./coverage');
...
async function handleConfirmCoverageButton(interaction) {
  ...
  // Single-role path: fetch ✅ reactors
...
async function handleMultiRoleButton(interaction, gameId, game) {
  ...
  // Build candidate list from ✅ reactors
  const message     = interaction.message;
  const yesReaction = message.reactions.cache.find(r => r.emoji.name === '✅');
  let candidates    = [];
...
async function handleCovCancelButton(interaction) {
  ...
  if (type === 'shift') {
    ...
    if (request.status === 'cancelled') return interaction.editReply('❌ Already cancelled.');
    ...
    console.log(`[confirm] ${interaction.user.tag} cancelled coverage request ${request.id} (shift ${id}) via /open-coverage`);
    await interaction.editReply(`✅ Shift \`${id}\` and its coverage request cancelled — post(s) deleted.`);
  } else {
    const game = db.getCustomGameById(id);
    if (!game) return interaction.editReply('❌ Game not found.');
    if (game.filled_at) return interaction.editReply('❌ This game is already closed.');
