...
async function handleCovCancelButton(interaction) {
  ...
  if (type === 'shift') {
    ...
    // If this was the last open shift, cancel the whole request and clean up the header
    const remaining = db.getCoverageShiftsByRequest(request.id).filter(s => s.status === 'open');
    if (remaining.length === 0) {
      db.markRequestCancelled(request.id);
      // Delete the header post if it's a separate message from the one we just deleted
