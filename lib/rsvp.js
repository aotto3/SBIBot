...
const { SHOWS, allEmojisForShow, emojiDisplay, getShowRole, showLabel, getRoleCoverage, ALL_SHOW_EMOJI_NAMES } = require('./shows');
...
async function updateCustomGameTracker(message, gameRecord) {
  ...
  if (config.roleGroups) {
    ...
    for (const group of config.roleGroups) {
      ...
      for (const [, u] of yesUsers) {
        ...
        if (roleStr && roleStr.split('/').includes(group.role)) {
          availableNames.push(db.getMemberFirstName(u.id, u.displayName ?? u.username));
        }
      }
      availableNames.sort();
...
async function checkFilled(message, gameRecord, guild) {
  ...
  // Resolve display names and build "Alice (Daphne) and Bob (Houdini)" style string
  const castList = cast
    .map(({ userId, role }) => {
      const u         = yesUsers.get(userId);
      const firstName = db.getMemberFirstName(userId, u?.displayName ?? u?.username);
  ...
  const dmContent   = `<@${fresh.requester_id}>, ${castStr} ${verb} available for **${showLabel(fresh.show)}** on ${dateTimeStr}!`;
