...
const { allEmojisForShow, emojiDisplay, getShowRole, showLabel, showEmojis, showRoleGroups, ALL_SHOW_EMOJI_NAMES } = require('./shows');
...
// ─── Recurring meetings: shared per-occurrence RSVP ───────────────────────────

/**
 * Handle a reaction on any meeting reminder post. For recurring (weekly or
 * monthly) 7d/24h posts, the two posts share one occurrence RSVP: reacting on
 * either updates the shared state (most-recent-wins), removes the now-stale
 * reaction from the other post, and re-renders both trackers. All other meeting
 * posts fall back to the single-message tracker.
 */
async function handleMeetingReaction(client, reaction, user, action, reminderRecord) {
  ...
  const isRecurringOccurrence = meeting
...
async function handlePingRedirect(client, reaction, user, pingRecord) {
  ...
  if (!pingRecord.redirected) {
    ...
    const newContent   = `${mentionsPart} Reminder: coverage still needed${forPart} — React ASAP, to the original message located at: ${link}`;
    try {
