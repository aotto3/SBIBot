...
async function postMeetingReminder(client, meeting, instanceDate, reminderType) {
  ...
  if (isMonthlyRecurring && reminderType === '7d') {
    // Monthly recurring: the 7-day post is this occurrence's RSVP post.
    const { content, includeRsvp } = buildMeetingReminderContent(meeting, instanceDate, '7d');
    msgContent   = content;
    addReactions = includeRsvp;
  } else if (reminderType === '7d' || reminderType === '24h') {
    // Follow-up post. Monthly 24h links to this occurrence's 7-day RSVP post;
    // one-time and weekly reminders link to the 'created' post (unchanged).
    const sourceRecord = (isMonthlyRecurring && reminderType === '24h')
      ? db.getReminderRecord(meeting.id, instanceDate, '7d')
      : db.getCreatedReminderRecord(meeting.id, instanceDate);
...
async function editMeetingPosts(client, meeting, oldChannelId) {
  ...
  for (const rec of records) {
    try {
      ...
      const isMonthlyRecurring = meeting.recurrence_type === 'monthly_weekday';
      // A "created" post and a monthly meeting's "7d" post are both RSVP posts
      // (reactions + live tracker); everything else is a follow-up.
      const isRsvpPost = rec.reminder_type === 'created' || (isMonthlyRecurring && rec.reminder_type === '7d');

      if (!isRsvpPost && (rec.reminder_type === '7d' || rec.reminder_type === '24h')) {
        const sourceRecord = (isMonthlyRecurring && rec.reminder_type === '24h')
          ? db.getReminderRecord(meeting.id, rec.instance_date, '7d')
          : db.getCreatedReminderRecord(meeting.id, rec.instance_date);
