...
const { buildSharedRsvpTracker } = require('./meeting-rsvp');
 ...
 */
function buildMeetingReminderContent(meeting, instanceDate, reminderType, crossPostUrl = null) {
  let ping;
  ...
  if (includeRsvp) {
    ...
    if (crossPostUrl) lines.push(`_You can also RSVP on the 7-day post: ${crossPostUrl}_`);
  }
...
/**
 * Fetch attendee mentions and a permalink for a prior post of this occurrence
 * (the 'created' announcement, or the '7d' RSVP post for monthly meetings).
 * Returns empty mentions and a null URL if the source post can't be found.
 *
 * @param {import('discord.js').TextChannel} channel
 * @param {string} channelIdForUrl  Channel ID to embed in the permalink
 * @param {object|null|undefined} sourceRecord  meeting_reminders_sent row (or falsy)
 * @returns {Promise<{ attendeeMentions: string[], originalUrl: string|null }>}
 */
async function fetchOccurrenceAttendees(channel, channelIdForUrl, sourceRecord) {
  let attendeeMentions = [];
  let originalUrl = null;
  if (sourceRecord && sourceRecord.message_id) {
    try {
      const originalMsg = await channel.messages.fetch(sourceRecord.message_id);
      originalUrl = `https://discord.com/channels/${channel.guildId}/${channelIdForUrl}/${sourceRecord.message_id}`;
      const ids = await fetchAttendeeIds(originalMsg);
      attendeeMentions = ids.map(id => `<@${id}>`);
    } catch {
      // source post unavailable — post without link or mentions
    }
  }
  return { attendeeMentions, originalUrl };
}
 ...
 *
 * For monthly recurring meetings the 7-day post is this occurrence's RSVP post
 * (reactions + live tracker) and the 24-hour post links back to it. One-time and
 * weekly meetings keep the original behavior (7d/24h link to the 'created' post).
 *
...
async function postMeetingReminder(client, meeting, instanceDate, reminderType) {
  ...
  const isRecurring         = !!meeting.recurrence_type;
  const isRecurringRsvpPost = isRecurring && (reminderType === '7d' || reminderType === '24h');

  let msgContent;
  let addReactions = false;

  if (isRecurringRsvpPost) {
    // Recurring (weekly or monthly): both the 7-day and 24-hour posts are RSVP
    // posts that share one occurrence RSVP state (reactions + one synced tracker).
    // The 24-hour post also links to the 7-day. Each post is keyed by this
    // occurrence's instance_date, so it works for every occurrence — unlike the
    // one-time follow-up path, which can only link to the single 'created' post.
    let crossPostUrl = null;
    if (reminderType === '24h') {
      const sevenDay = db.getReminderRecord(meeting.id, instanceDate, '7d');
      if (sevenDay && sevenDay.message_id) {
        crossPostUrl = `https://discord.com/channels/${channel.guildId}/${meeting.channel_id}/${sevenDay.message_id}`;
      }
    }
    const { content, includeRsvp } = buildMeetingReminderContent(meeting, instanceDate, reminderType, crossPostUrl);
    msgContent   = content;
    addReactions = includeRsvp;
  } else if (reminderType === '7d' || reminderType === '24h') {
    // One-time meetings: the 7d/24h reminders link back to the 'created' RSVP post
    // (same occurrence, so getCreatedReminderRecord resolves).
    const sourceRecord = db.getCreatedReminderRecord(meeting.id, instanceDate);
    const { attendeeMentions, originalUrl } = await fetchOccurrenceAttendees(channel, meeting.channel_id, sourceRecord);
  ...
  // Render the shared RSVP tracker onto a recurring post immediately, so a freshly
  // posted 24-hour reminder shows RSVPs already made on the 7-day post.
  if (isRecurringRsvpPost) {
    const rows = db.getMeetingRsvps(meeting.id, instanceDate);
    try {
      await msg.edit(`${msgContent}${TRACKER_MARKER}${buildSharedRsvpTracker(rows)}`);
    } catch (err) {
      console.warn(`[meetings] Could not render initial tracker on ${reminderType} post:`, err.message);
    }
  }
...
async function editMeetingPosts(client, meeting, oldChannelId) {
  ...
  for (const rec of records) {
    try {
      ...
      const isRecurring = !!meeting.recurrence_type;
      // A "created" post and a recurring meeting's "7d"/"24h" posts are all RSVP
      // posts (reactions + live tracker); everything else is a follow-up.
      const isRecurringRsvpPost = isRecurring && (rec.reminder_type === '7d' || rec.reminder_type === '24h');
      const isRsvpPost = rec.reminder_type === 'created' || isRecurringRsvpPost;

      if (!isRsvpPost && (rec.reminder_type === '7d' || rec.reminder_type === '24h')) {
        // One-time follow-up: link to the 'created' post.
        const sourceRecord = db.getCreatedReminderRecord(meeting.id, rec.instance_date);
        const { attendeeMentions, originalUrl } = await fetchOccurrenceAttendees(channel, oldChannelId, sourceRecord);
        newContent = buildFollowupReminderContent(meeting, rec.instance_date, rec.reminder_type, attendeeMentions, originalUrl);
      } else {
        // RSVP post — preserve the live RSVP tracker section, rebuild the header.
        let crossPostUrl = null;
        if (isRecurringRsvpPost && rec.reminder_type === '24h') {
          const sevenDay = db.getReminderRecord(meeting.id, rec.instance_date, '7d');
          if (sevenDay && sevenDay.message_id) {
            crossPostUrl = `https://discord.com/channels/${channel.guildId}/${oldChannelId}/${sevenDay.message_id}`;
          }
        }
        const parts = msg.content.split(TRACKER_MARKER);
        const { content: newHeader } = buildMeetingReminderContent(meeting, rec.instance_date, rec.reminder_type, crossPostUrl);
...
module.exports = { postMeetingReminder, editMeetingPosts, buildMeetingReminderContent, buildFollowupReminderContent, buildCancelledPostContent, fetchAttendeeIds, RSVP_EMOJIS, TRACKER_MARKER };
