...
async function postMeetingReminder(client, meeting, instanceDate, reminderType) {
  ...
  const isMonthlyRecurring = meeting.recurrence_type === 'monthly_weekday';
  const isMonthlyRsvpPost  = isMonthlyRecurring && (reminderType === '7d' || reminderType === '24h');

  let msgContent;
  let addReactions = false;

  if (isMonthlyRsvpPost) {
    // Monthly recurring: both the 7-day and 24-hour posts are RSVP posts that
    // share one occurrence RSVP state. The 24-hour post also links to the 7-day.
    ...
    // One-time and weekly reminders link to the 'created' post (unchanged).
  ...
  // Render the shared RSVP tracker onto a monthly post immediately, so a freshly
...
async function editMeetingPosts(client, meeting, oldChannelId) {
  ...
  for (const rec of records) {
    try {
      ...
      const isMonthlyRecurring = meeting.recurrence_type === 'monthly_weekday';
      // A "created" post and a monthly meeting's "7d"/"24h" posts are all RSVP
      // posts (reactions + live tracker); everything else is a follow-up.
      const isMonthlyRsvpPost = isMonthlyRecurring && (rec.reminder_type === '7d' || rec.reminder_type === '24h');
      const isRsvpPost = rec.reminder_type === 'created' || isMonthlyRsvpPost;

      if (!isRsvpPost && (rec.reminder_type === '7d' || rec.reminder_type === '24h')) {
        // One-time / weekly follow-up: link to the 'created' post.
        ...
        let crossPostUrl = null;
        if (isMonthlyRsvpPost && rec.reminder_type === '24h') {
