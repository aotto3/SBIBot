...
function computeCoverageStats(rows = [], opts = {}) {
  ...
  const filtered = _applyRowFilters(rows, opts); // show + since (person only scopes the detail)
  ...
  return {
    leaderboard,
    fillRate:   _computeFillRate(filtered, now),
    timing:     _computeTiming(filtered),
    mostNeeded: _computeMostNeeded(filtered),
    person:     opts.person ? _computePersonDetail(filtered, opts.person, now) : null,
 ...
 */
function _computePersonDetail(rows, id, now) {
  ...
  for (const row of rows) {
    if (row.confirmed_taker_id === id) {
      covers += 1;
      coverShowCount.set(row.show, (coverShowCount.get(row.show) ?? 0) + 1);
    }
