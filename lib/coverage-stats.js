 ...
 * buckets. Slice #146 adds timing (time-to-coverage + lead time). A later
 * slice adds most-needed shifts.
 */
...
function computeCoverageStats(rows = [], opts = {}) {
  ...
  return {
    leaderboard,
    fillRate: _computeFillRate(rows, opts.now ?? Date.now()),
    timing:   _computeTiming(rows),
  };
}
