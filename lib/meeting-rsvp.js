...
/**
 * Build the RSVP tracker text from shared per-occurrence RSVP rows. Pure.
 * Both the 7-day and 24-hour posts render the same tracker from this state.
 *
 * @param {Array<{status:'yes'|'maybe'|'no', user_id:string, display_name?:string|null}>} rows
 * @returns {string}
 */
function buildSharedRsvpTracker(rows) {
  const byStatus = { yes: [], no: [], maybe: [] };
  for (const r of rows) {
    if (byStatus[r.status]) byStatus[r.status].push(r.display_name || r.user_id);
  }
  for (const k of Object.keys(byStatus)) byStatus[k].sort();
  return [
    `✅ **Attending (${byStatus.yes.length}):** ${byStatus.yes.length ? byStatus.yes.join(', ') : '_none yet_'}`,
    `❌ **Not attending (${byStatus.no.length}):** ${byStatus.no.length ? byStatus.no.join(', ') : '_none yet_'}`,
    `❓ **Maybe (${byStatus.maybe.length}):** ${byStatus.maybe.length ? byStatus.maybe.join(', ') : '_none yet_'}`,
  ].join('\n');
}

module.exports = { resolveReaction, statusForEmoji, buildSharedRsvpTracker, STATUS_EMOJI, EMOJI_STATUS };
