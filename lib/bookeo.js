 ...
 * @param {string} [from] - YYYY-MM-DD start date, inclusive (defaults to today on the server)
 * @param {string} [to]   - YYYY-MM-DD end date, INCLUSIVE (defaults to from+7 on the server).
 *                          Sent as an exclusive `end = to + 1 day` on the wire — see below.
...
async function getSchedule(from, to) {
  ...
  // IMPORTANT: the live bookeo-asst /api/schedule endpoint reads the date range
  // from `start`/`end` query params (YYYY-MM-DD) — NOT `from`/`to`. All verified
  // directly against the live API (2026-09-08); do NOT change any of this without
  // re-verifying, because getting it wrong silently returns the wrong data:
  //   • `from`/`to` are silently ignored — you get the server's default ~7-day
  //     window regardless. This was the original bug.
  //   • `end` is EXCLUSIVE and must be strictly after `start` (start === end
  //     returns HTTP 500 "startTime must be before endTime"). This function's
  //     contract, relied on by every caller, is that `to` is INCLUSIVE (e.g.
  //     getSchedule(today, today) must return today's shifts), so we send
  //     end = to + 1 day. This also makes same-day requests valid.
  //   • Hard 31-day max span per call (endTime - startTime): a wider range
  //     returns HTTP 500 "The maximum date range per call is 31 days ...".
  //     Counted by calendar date (DST-independent). getScheduleForDays chunks
  //     so the +1 day above never pushes a chunk past this (see CHUNK_DAYS).
  const params = {};
  if (from) params.start = from;
  if (to) {
    const [ty, tm, td] = to.split('-').map(Number);
    params.end = toDateString(new Date(ty, tm - 1, td + 1));
  }
...
// The bookeo-asst /api/schedule endpoint enforces a hard 31-day max span per
// call (a wider start/end range returns HTTP 500), so a longer horizon is
// stitched from several chunks of this size, merged together. Each chunk spans
// CHUNK_DAYS inclusive days; getSchedule sends end = to + 1 day (see above), so
// the wire span is CHUNK_DAYS + 1 = 31 calendar days — exactly the documented
// max, verified against the live API to return 200 (the limit counts calendar
// dates, so it holds even across a DST boundary). This covers a 90-day horizon
// in just 3 calls instead of 13. Do not raise it: 31 already sits at the cap.
const CHUNK_DAYS = 30;
 ...
 * date+time+show and clipped to [from, to]. Chunk ranges are contiguous (each
 * chunk's `to` is the next chunk's `from`) and the final chunk can extend past
 * the requested `to`, so rows outside [from, to] are dropped at the edges.
 *
...
/**
 * Fetch the schedule across a horizon longer than a single bookeo-asst call
 * allows (it caps each call at a 31-day start/end span). Issues one call per
 * CHUNK_DAYS-day chunk in parallel (each chunk still benefits from
 * getSchedule's normal 5-min cache) and merges the results.
 *
