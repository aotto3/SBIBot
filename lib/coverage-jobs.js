...
const { showAutoRole, showCharacters, getDiscordRoleName, getShowRole, showEmojis, showReactionList } = require('./shows');
const { planMissingRolePings, buildEodDM, buildAllRespondedDM, analyzeCoverage } = require('./coverage');
const { extractMaybeReactorIds, planMaybeNudges, buildMaybeNudgeDM } = require('./maybe-nudge');
...
/**
 * Choose which planned pings to actually send.
 * Pure — no I/O.
 *
 * - 'all'   → every planned item (the scheduled 8am behavior).
 * - 'smart' → only items whose post has NOT already been pinged today, so a manual
 *             re-run picks up exactly the drops instead of re-pinging everyone.
 *
 * @param {Array<{ messageId: string }>} plan
 * @param {string[]} alreadyPingedMessageIds  Original message ids pinged today
 * @param {'all'|'smart'} mode
 */
function planCoveragePingTargets(plan, alreadyPingedMessageIds = [], mode = 'all') {
  if (mode === 'smart') {
    const pinged = new Set(alreadyPingedMessageIds);
    return plan.filter(p => !pinged.has(p.messageId));
  }
  return plan;
}

/**
 * Fetch a guild's member roster with bounded retry + backoff.
 *
 * The underlying fetch is a gateway opcode-8 request that Discord rate-limits and
 * that can also time out ("Members didn't arrive in time"). A single transient
 * failure would otherwise abort the whole run and drop every reminder, so retry a
 * few times before giving up. `sleep` is injectable for deterministic tests.
 *
 * @param {{ fetchGuildMembers: (guild: any) => Promise<any> }} discord
 * @param {any} guild
...
async function fetchGuildMembersWithRetry(
  discord,
  guild,
  { retries = 2, baseDelayMs = 1500, sleep = ms => new Promise(r => setTimeout(r, ms)) } = {},
) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await discord.fetchGuildMembers(guild);
    } catch (err) {
      lastErr = err;
      if (attempt < retries) {
        console.warn(`[scheduler] guild member fetch failed (attempt ${attempt + 1}/${retries + 1}), retrying: ${err.message}`);
...
async function runCoverageRolePings(discord, repo, opts = {}) {
  const { mode = 'all', preview = false, alreadyPingedMessageIds = [] } = opts;
  const openShifts   = repo.getOpenShifts();
  const openGames    = repo.getOpenGames();
  if (!openShifts.length && !openGames.length) return { planned: 0, sent: 0, failed: [] };

  const exclusionIds = db.getCoveragePingExclusions();

  // Maps: messageId → show / dateTimeStr / date / time / requester_id / requester_name / shift meta
  const showByMessageId          = new Map();
  const dateTimeByMessageId      = new Map();
  const dateByMessageId          = new Map();
  const timeByMessageId          = new Map();
  ...
  for (const s of openShifts) {
    ...
    dateByMessageId.set(s.shift_message_id, s.date);
    timeByMessageId.set(s.shift_message_id, s.time);
  ...
  for (const g of openGames) {
    ...
    dateByMessageId.set(g.message_id, g.date);
    timeByMessageId.set(g.message_id, g.time);
    enrichedGames.push({ ...g, missingRoles });
  }

  const plan = planMissingRolePings(enrichedShifts, enrichedGames);
  if (!plan.length) return { planned: 0, sent: 0, failed: [] };

  // 'smart' mode (manual re-run) skips posts already pinged today; 'all' keeps every item.
  const targets = planCoveragePingTargets(plan, alreadyPingedMessageIds, mode);
  if (!targets.length) return { planned: 0, sent: 0, failed: [] };

  // Preview: report what would be pinged without sending anything.
  if (preview) {
    return {
      ...
      preview: targets.map(t => ({
        ...
        show:      showByMessageId.get(t.messageId),
        dateTime:  dateTimeByMessageId.get(t.messageId),
      })),
    };
  }

  console.log(`[scheduler] Sending ${targets.length} missing-role ping(s)`);

  // Hydrate each guild's roles + members exactly once per run. Fetching the full
  // member roster is a gateway opcode-8 request that Discord rate-limits hard; doing
  // it once per reminder (as before) dropped all but the first ping each morning.
  const hydratedGuilds = new Set();
  let   sent   = 0;
  const failed = [];

  for (const { channelId, roleNames, messageId } of targets) {
    try {
      ...
      if (mentions.length > 0) {
        ...
        const reactionList = show ? showReactionList(guild, show) : '✅';
        const content      = `${mentions.join(' ')} Reminder: coverage still needed${forPart} — react ${reactionList} to the original request ASAP: ${link}`;
        const pingMsg     = await discord.sendMessage(ch, content);
        console.log(`[scheduler] Sent role ping in channel ${channelId} for message ${messageId}`);
        try {
          repo.savePingMessage(
            pingMsg.id,
            messageId,
            channelId,
            showByMessageId.get(messageId),
            dateByMessageId.get(messageId),
            timeByMessageId.get(messageId),
          );
        } catch (err) {
          console.warn(`[scheduler] Could not save ping message record for ${pingMsg.id}:`, err.message);
        }
      }
      ...
      failed.push({ id: messageId, type: 'coverage-ping', reason: err.message });
    }
  }

  return { planned: targets.length, sent, failed };
}
...
// ─── Weekly maybe-nudge execute ───────────────────────────────────────────────

/**
 * Sunday job: DM anyone still marked "maybe" on an open (uncovered) coverage
 * shift or custom game, asking them to firm up to ✅/❌. One consolidated DM per
 * person. Covered/confirmed/filled/cancelled posts are already excluded (they
 * aren't returned by the open-post queries); past-dated posts and excluded
 * users are filtered by planMaybeNudges.
 */
async function runMaybeNudge(discord, repo) {
  const openShifts = repo.getOpenShifts();
  const openGames  = repo.getOpenGames();
  ...
  const rawPosts = [
    ...openShifts.map(s => ({ show: s.show, date: s.date, time: s.time, channelId: s.channel_id, messageId: s.shift_message_id })),
    ...openGames.map(g  => ({ show: g.show, date: g.date, time: g.time, channelId: g.channel_id, messageId: g.message_id })),
  ];

  const posts = [];
  for (const p of rawPosts) {
    if (!p.messageId || !p.channelId) continue;
    try {
      const ch    = await discord.fetchChannel(p.channelId);
      const guild = ch.guild;
      const msg   = await discord.fetchMessage(ch, p.messageId);

      const reactions = [];
      for (const reaction of msg.reactions.cache.values()) {
        const users   = await discord.fetchReactionUsers(reaction);
        const userIds = [...users.values()].filter(u => !u.bot).map(u => u.id);
        reactions.push({ name: reaction.emoji.name, userIds });
      }

      const maybeUserIds = extractMaybeReactorIds(reactions, p.show);
      if (!maybeUserIds.length) continue;

      posts.push({
        show: p.show,
        date: p.date,
        time: p.time,
        link: `https://discord.com/channels/${guild.id}/${p.channelId}/${p.messageId}`,
        maybeUserIds,
      });
    } catch (err) {
      console.warn(`[maybe-nudge] Could not read reactions for message ${p.messageId}:`, err.message);
    }
  }

  const byUser = planMaybeNudges(posts, exclusionIds, todayStr);
  if (!byUser.size) {
    console.log('[maybe-nudge] No maybe responders to nudge');
    return;
  }
  ...
  for (const [userId, items] of byUser) {
    try {
      await discord.sendDM(userId, buildMaybeNudgeDM(items));
      sent++;
    } catch (err) {
      console.warn(`[maybe-nudge] Could not DM ${userId}:`, err.message);
    }
  }
  console.log(`[maybe-nudge] Sent ${sent} maybe-nudge DM(s) for ${byUser.size} recipient(s)`);
}
