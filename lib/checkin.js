...
function isEligibleForCheckin(shift) {
  const config = SHOWS[shift.show];
  return !!(config && config.checkin && shift.cast.length > 0);
}
...
async function seedTodayCheckins(client) {
  ...
  for (const group of groups) {
    ...
    const showConfig = SHOWS[show].checkin;
...
async function fireCheckinAlert(client, rec) {
  ...
  const content = `⚠️ ${pingLine} **${rec.bookeo_name}** has not checked in for **${SHOWS[rec.show].label}**. Call time was ${callTimeStr}.`.trim();
