...
const { showLabel, hasCheckin, checkinConfig, getShowRole } = require('./shows');
const { CENTRAL_TZ } = require('./utils');
const cfg = require('./config');
const members = require('./members');

// Tracks record IDs that already have a timeout scheduled in this process.
// Prevents double-scheduling when seedAndScheduleToday() is called more than
// once in the same run (e.g. midnight cron fires, then the bot redeploys).
const _scheduledAlerts = new Set();

// Retry constants shared by startup and the nightly re-seeding cron.
const SEED_TIMEOUT_MS  = 20_000;
const SEED_RETRY_MS    = 5 * 60 * 1000;
const SEED_MAX_RETRIES = 12; // 12 × 5 min = 1 hour
 ...
 * Does NOT schedule timeouts — call scheduleAlerts() separately for that.
 *
 * @param {object} client             Discord client
 * @param {object} [opts]
 * @param {object} [opts._bookeo]     Bookeo module override (for testing)
 * @returns {Promise<number>} Number of records upserted
 */
async function seedToday(client, { _bookeo } = {}) {
  const bk        = _bookeo || bookeo;
  ...
  let shifts;
  try {
    shifts = await bk.getSchedule(today, today);
  } catch (err) {
    console.error('[checkin] Failed to fetch schedule for seeding:', err.message);
    return 0;
  }

  // Defensively narrow to today's rows: bookeo-asst is a third-party dependency,
  // so we don't blindly trust its range scoping even now that start/end works.
  ...
  const errorChannelId = cfg.getErrorChannelId();
  ...
  for (const group of groups) {
    ...
    // Belt-and-suspenders: groupEligibleShifts already filters via isEligibleForCheckin,
    // but guard here so a future config inconsistency skips only this group rather than
    // crashing showConfig.roles.some() and aborting the entire seeding loop.
    if (!showConfig) {
      const msg = `⚠️ [check-in] Show **${show}** passed eligibility filter but has no checkin config — skipping **${bookeo_name}**. Check shows.js and bookeo.js abbreviation map.`;
      console.error('[checkin] CONFIG BUG:', msg);
      if (errorChannelId) {
        try {
          const ch = await client.channels.fetch(errorChannelId);
          await ch.send(msg);
        } catch (e) {
          console.error('[checkin] Failed to post config-bug notice to error channel:', e.message);
        }
      }
      continue;
    }
    ...
    const member = members.resolveByBookeoName(bookeo_name);
    if (!member) {
    ...
    const roleStr = await getShowRole(guild, member.discordId, show);
...
/**
 * Schedule no-show alerts for a list of pending checkin records.
 * Records already scheduled in this process (tracked by _scheduledAlerts) are skipped.
 * Call after seedToday() to avoid triggering timers during the seeding step.
 *
 * @param {object}   client   Discord client
 * @param {object[]} records  Checkin record rows to schedule alerts for
 */
function scheduleAlerts(client, records) {
  for (const rec of records) {
    _scheduleCheckinAlert(client, rec);
  }
  if (records.length) {
    console.log(`[checkin] ${records.length} alert(s) scheduled`);
  }
}
 ...
 */
async function seedAndScheduleToday(client) {
  const seeded = await seedToday(client);
  ...
  // record's alert is only scheduled once per process.
  const today   = utils.todayCentral();
  const pending = db.getPendingCheckins(today);
  scheduleAlerts(client, pending);

  const scheduled = pending.length;
  if (scheduled) {
    console.log(`[checkin] ${scheduled} alert(s) total (${seeded} new record(s) seeded today)`);
  }
 ...
 */
async function _fireCheckinAlert(client, rec) {
  // Re-fetch the record — it may have been checked in or already alerted
  // while the timeout was pending (e.g. double-scheduled after a redeploy).
  const fresh = db.getCheckinRecordById(rec.id);
  if (!fresh) return;
  const state = db.getCheckinState(fresh);
  if (state === 'checked-in') {
    console.log(`[checkin] Alert suppressed for ${rec.bookeo_name} / ${rec.show} — already checked in`);
    return;
  }
  if (state === 'alerted') {
    console.log(`[checkin] Alert suppressed for ${rec.bookeo_name} / ${rec.show} — alert already posted`);
    return;
  }
  ...
  try {
    const guild   = client.guilds.cache.get(process.env.DISCORD_GUILD_ID);
    const channel = await utils.resolveCheckinChannel(guild, rec.show);
    const msg     = await channel.send(content);
    db.storeAlertInfo(rec.id, msg.id, channel.id);
 ...
 * @param {object} rec  A checkin_records row
 */
function _scheduleCheckinAlert(client, rec) {
  if (_scheduledAlerts.has(rec.id)) {
    console.log(`[checkin] Alert for ${rec.bookeo_name} / ${rec.show} already scheduled in this run — skip`);
    return;
  }
  _scheduledAlerts.add(rec.id);
  ...
  if (delayMs > 0) {
    // unref so a pending alert timer (which can be hours out) never keeps a
    // short-lived process (e.g. the test runner) alive after its work is done.
    setTimeout(() => _fireCheckinAlert(client, rec), delayMs).unref();
 ...
 */
async function _editAlertForLateCheckin(rec, forcedById = null, client) {
 ...
 */
async function performCheckin(recordId, { forcedBy = null } = {}, client) {
  const rec = db.getCheckinRecordById(recordId);
  if (!rec) return { status: 'not_found' };
  if (db.getCheckinState(rec) === 'checked-in') return { status: 'already_checked_in' };

  db.markCheckedIn(recordId, forcedBy);
  const fresh = db.getCheckinRecordById(recordId);
  await _editAlertForLateCheckin(fresh, forcedBy, client);
...
/**
 * Count today's check-in records that haven't been completed yet (pending +
 * already-alerted no-shows). Used by /bot-status's data-at-a-glance section.
 *
 * @param {string} [date]  YYYY-MM-DD (defaults to today Central)
 * @returns {number}
 */
function countPendingCheckinsToday(date = utils.todayCentral()) {
  return db.getCheckinRecordsByDateRange(date, date).filter(r => !r.checked_in_at).length;
}
...
/**
 * Seed today's check-in records with retry on failure.
 * Wraps seedAndScheduleToday() in a timeout + exponential-ish backoff so
 * transient Bookeo downtime at startup or midnight doesn't lose the day's records.
 *
 * @param {string} seedDate  The YYYY-MM-DD string this run is seeding for
 * @param {number} attempt   Current attempt number (1-based)
 */
async function seedWithRetry(client, seedDate, attempt = 1) {
  try {
    let seedTimeout;
    try {
      await Promise.race([
        ...
        new Promise((_, reject) => {
          seedTimeout = setTimeout(() => reject(new Error('timed out after 20s')), SEED_TIMEOUT_MS);
        }),
      ]);
    } finally {
      // Clear the losing timer so it neither keeps the process alive nor rejects late.
      clearTimeout(seedTimeout);
    }
    // Schedule alerts only once on success — not on each retry attempt.
    const today   = utils.todayCentral();
    const pending = db.getPendingCheckins(today);
    scheduleAlerts(client, pending);
    ...
    setTimeout(() => seedWithRetry(client, seedDate, attempt + 1), SEED_RETRY_MS).unref();
  }
}
